...
if [ "$NDK_USR_LIB" = "" ]; then
    NDK_USR_LIB=$NDK/toolchains/llvm/prebuilt/$HOST_TAG/sysroot/usr/lib/i686-linux-android/$BUILD_ANDROID_PLATFORM
fi
...
./configure --logfile=conflog.txt --target-os=linux \
    ...
    --arch=x86 \
    ${GENERAL} \
    --extra-cflags=" --target=$TARGET -no-canonical-prefixes -fdata-sections -ffunction-sections -fno-limit-debug-info -funwind-tables -fPIC -O3 -DCONFIG_PIC -DANDROID -DANDROID_PLATFORM=android-$BUILD_ANDROID_PLATFORM -Dipv6mr_interface=ipv6mr_ifindex -fasm -fno-short-enums -fno-strict-aliasing -fomit-frame-pointer -march=k8" \
    --disable-shared \
    --enable-static \
...
echo Android x86 build finished